...
`git reflog` - показать коммиты в т.ч. скрытые за счет amend, показывает журнал текущего окружения, фиксирует все измения в локальном репозитории, в т.ч. историю перемещения заголовков
...
`git show HEAD|[sha1]|[branch]^` - показать родителя текущего выбранного коммита, родителя комитта с хешем _sha1_ или родителя ветки _branch_ (**показывает только одного родителя**)

`^n` - показать n предков

`~n` - посмотреть предродителя в n поколении
...
`git log <branch1>...<branch2>` - отобразить коммиты которые есть в _branch1_, в _branch2_ исключив те что есть в обоих ветках сразу, опция `--left-right` укажет к какой ветке относится какой коммит
